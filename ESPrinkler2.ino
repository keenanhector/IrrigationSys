...
void handleFileUpload2(){ // upload a new file to the SPIFFS
  ...
  if(upload.status == UPLOAD_FILE_START){
    ...
    if(fsUploadFile) {                                    // If the file was successfully created
      ...
      server.sendHeader("Location","/image.html");      // Redirect the client to the success page
      server.send(303);
    } else {
...
void setRelays() {
  ...
  Serial.println("Sending Relay Data");
...
void setup(void) {
  ...
  u8g2.clearBuffer();
  displayStatus(0, "Start %s", __VERSION__);
